...
class HWD():
    ...
    def standard_case(cls, _id: int = np.random.SeedSequence().entropy):
        """Alternative initialiser with the default values

        Args:
            _id (int, optional): Random seed. Defaults to np.random.SeedSequence().entropy.
    ...
    def event_file(
        ...
        """Load the event descriptions defined in *file_name* and *sheet_name*.

        Args:
            file_name (Optional[str], optional): Path to the file with events. Defaults to None.
            sheet_name (str, optional): Sheet name. Defaults to "Basic".

        Returns:
            pd.DataFrame: A dataframe with the event descriptions
        """
    ...
    def interday_distribution(
        ...
        Args:
            list_dates (list | pd.DataFrame | pd.DatetimeIndex): List of dates, or a DatetimeIndex to define the interday distribution. If a Dataframe is given, the index is used (expected DatetimeIndex)
            sample_file (Optional[str], optional): If self.daily_distribution="sample", it uses this file path to get the distribution. Defaults to None.

        Raises:
            ValueError: In case, the self.daily_distribution is not a valid value.

        Returns:
            pd.DataFrame: Dataframe with the daily HWD (L/day) for all the given dates.
        """
    ...
    def generator(
        ...
        """Main function to generate HWD data. It calls either *generator_standard* or *generator_events*.

        Args:
            timeseries (pd.DataFrame | pd.DatetimeIndex): Timeseries dataframe or index to be filled.
            method (str, optional): Method used to generate data. "standard" or "events". Defaults to 'standard'.
            interday_dist (Optional[pd.DataFrame], optional): Interday distribution. Usually a dataframe generated by *HWD.interday_distribution* method. Defaults to None.
            intraday_dist (Optional[int], optional): The HWDP profile, as defined in the project. Values 1 to 6. Defaults to None.
            event_probs (Optional[pd.DataFrame], optional): The events probabilities, in case the "events" method is used. Usually defined by a file and loaded with the *"event_file"* method. Defaults to None.
            file_name (str, optional): See *event_file* method. Defaults to FILES_HWD_SAMPLES["HWD_events"].
            sheet_name (str, optional): See *event_file* method. Defaults to "Custom".
            columns (list[str], optional): Which columns to return. Defaults to SIMULATIONS_IO.TS_TYPES["HWDP"].

        Returns:
            pd.DataFrame: Dataframe with the HWD timeseries.
        """
