...
def main():
    ...
    sim.ts.STOP = Variable(100., "hr")
    sim.household.control_load = -1
    sim.household.control_random_on = False
    ...
    ts = general.load_timeseries_all(sim)
    ...
    if True:
        out_all_0D = tank_0D.SOC_profiles(sim.DEWH, ts, verbose=True)
        print(out_all_0D)
